...
import { LayoutDashboard, Users, Truck, LogOut, Settings, User as UserIcon, X } from "lucide-react";
...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <aside 
        className={`fixed left-0 top-0 h-screen w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col transition-all duration-300 z-50 
      ...
      >
        <div className="p-6 flex items-center justify-between gap-3">
          ...
          <button 
            onClick={close}
            className="p-2 md:hidden text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800 rounded-lg"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
